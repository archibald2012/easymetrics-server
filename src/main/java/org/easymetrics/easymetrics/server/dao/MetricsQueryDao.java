...
public class MetricsQueryDao {
	...
	private static final String	LOAD_DISTINCT_HOST				= "SELECT DISTINCT HOST_NAME FROM METRICS_RECORD where service_group=? and service = ?";
	...
	private static final String	LOAD_AGGREGATIONS_BY_HOST		= "SELECT AGG.AGGREGATION_ID, AGG.COMPONENT_NAME, AGG.FUNCTION_NAME, "
																		...
																		+ "WHERE REC.SERVICE_GROUP = ?" + "AND REC.DOMAIN = ?" + "AND REC.HOST_NAME = ?"
																		+ "AND REC.SERVICE = ?" + "AND REC.APPLICATION_VERSION = ?" + "AND REC.HOST_USER = ?"
																		+ "AND REC.INSTANCE_PID = ?" + "AND AGG.START_TIME >= ? AND AGG.START_TIME <= ?";

	private JdbcTemplate		jdbcTemplate;

	public List<Bucket> getBucketsByAggregationId(String aggregationId) {
		List<Bucket> result = jdbcTemplate.queryForList(LOAD_METRICS_BUCKET_BY_AGGRE, new Object[] { aggregationId }, Bucket.class);
		return result == null ? new ArrayList<Bucket>() : result;
	}

	public List<String> getDistinctHost(String serviceGroup, String service) {
		List<String> result = jdbcTemplate.queryForList(LOAD_DISTINCT_HOST, new Object[] { serviceGroup, service }, String.class);
	...
	public List<Aggregation> getAggregationsByHost(Record hostInfo, Date startDate, Date endDate) {
		List<Aggregation> result = jdbcTemplate.queryForList(LOAD_AGGREGATIONS_BY_HOST, new Object[] { hostInfo.getServiceGroup(), hostInfo.getDomain(),
				hostInfo.getHost(), hostInfo.getService(), hostInfo.getVersion(), hostInfo.getUser(), hostInfo.getPid(), startDate, endDate },
				Aggregation.class);
